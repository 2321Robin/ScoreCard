...
## 阶段 6：移动端优化与可访问性（已完成）
- 移动端：表格容器可横向滚动；新增/总分等主操作具备大点击区域；新增跳转主要内容的 skip link。
- 无障碍：表格添加语义 role/aria，输入错误态使用 aria-invalid，行合计 aria-live；按钮与快捷键补充 aria-label，焦点态统一 focus-visible 样式。
- 键盘操作：快捷按钮、自动平衡、复制、删除、导出等均可键盘聚焦操作；悬浮新增按钮可键盘触达。
...
## 测试
- 自动化：使用 Vitest + Testing Library。运行 `npm run test` 进入 watch，`npm run test:run` 无 UI 输出。
- 覆盖点：标题/主要按钮渲染，默认玩家加载，本地存储恢复的局数据可见。
- 手动检查（重点场景）：移动端横向滚动；长按快速加减；整局不平衡提示；自动平衡补差；撤销/重做；导出 CSV 文件名与内容；清空确认。
...
- 阶段 6：移动端优化与可访问性（已完成）
	...
	- 输出：在手机上流畅可用的版本。
- 阶段 7：测试与验收（已完成）
	- 自动化：基础渲染与本地存储恢复用例（Vitest + Testing Library）。
	- 手动测试清单：移动端输入、长按、导出、清空确认、撤销/重做。
	- 输出：测试运行脚本与检查清单。
- 阶段 8：优化与发布
