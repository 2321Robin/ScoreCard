...
## 功能概览
- 玩家管理：默认 4 人（玩家 A/B/C/D），2-8 人可增删改，自动同步表格列；支持重命名。新会话默认继承当前会话的玩家名单。
- 模式切换：积分模式（手动录入分值）、麻将模式（选择胡牌者、记录暗杠/点杠，按规则自动结算）、斗地主模式（选择地主与胜方，底分×倍数自动结算）；麻将固定 4 人、斗地主固定 3 人，切换模式时自动调整玩家数量并锁定增删。
...
- 斗地主模式：固定 3 人，每局点选地主、胜方（地主赢/农民赢）、底分、倍数（1/2/4/8/16 按钮）与封顶（默认 10，每局单人最多输）；地主赢则地主 +2×底分×倍数、两家农民各 −底分×倍数，农民赢则反向；单人输分超过封顶时按封顶结算，保持零和；对局记录展示地主/胜方/底分倍数/封顶徽标，总分卡展示地主赢/农民赢局数。
...
2) 斗地主：切换模式为“斗地主模式”（自动调整为 3 人），选择地主、胜方、底分与倍数，预览分数无误后点“添加本局”。
3) 编辑/删除：在对局表中点“编辑”进入可改状态，完成后“保存”或“取消”；可删除整局。
4) 撤销/重做：顶部按钮回溯或恢复最近 50 步操作；“清空”需确认。
5) 导出/导入：点“导出 CSV”下载，点“导入 CSV”选择此前导出的文件即可恢复。
6) 目标局数：顶部输入目标后点“设定”；达标会自动导出并提示是否继续，继续可输入新目标。
7) 自定义分值范围：在新增区域输入上下限并点“更新范围”。
8) 跨会话总览：在“指标”下拉选择总分/赢局/胡数/杠数，按需切换“范围”到自定义并勾选会话；表格与折线图会随选择更新，且只显示选中会话中出现过的玩家。
...
- 工具库：
  - [src/lib/constants.js](src/lib/constants.js)：全局常量与默认配置（玩家上限、模式常量、存储键、麻将/斗地主默认规则等）。
  ...
  - [src/lib/doudizhu.js](src/lib/doudizhu.js)：斗地主结算纯函数（底分×倍数自动计分、胜方推导）。
...
- 已完成：
  ...
  - 目录放在左侧可折叠，便于点击且不遮挡内容。
  - 修复展开目录后页面出现水平溢出的宽度问题。
  - 目录在页面滚动时保持可见（粘性定位）。
  - 调整目录与菜单不再重叠，并移除顶部的目录展开提示与按钮。
  - 修复修改玩家名字清空时被自动补全的 bug。
  - 导出 CSV 使用最后一局的添加时间作为文件名。
  - 删除清空按钮，优化顶部菜单布局。
  - 修复默认庄家逻辑，默认上一局赢家为庄家。
  - 添加清水胡选项（点按启用/取消，胡牌者 +3，其余 -1，规则计分项）。
  - 添加抢杠选项：选择 A 抢杠 B，则 A（胡牌者）+3、B -3，其余 0，可叠加其它麻将调整，预览与编辑即时反映。
  - 新增斗地主模式：固定 3 人，选择地主/胜方/底分/倍数自动零和结算，支持编辑与统计（地主赢/农民赢局数）。
  ...
  - 移除总分旁的“提示”模块。
  - 麻将/斗地主模式的玩家选择（胡牌者、庄家、抢杠、地主等）、买码、跟庄类型等下拉改为“点一次”的胶囊按钮组，再点一次可取消；编辑面板同步改造。
  - 斗地主胜方改为按钮组；倍数改为 1/2/4/8/16 按钮；新增封顶（每局单人最多输，默认 10，可改），超限按封顶结算保持零和。
- 未完成
  - 无
